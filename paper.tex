...
Re_L = \frac{UL}{\nu},
\label{eq:Re}
\end{equation}\textbf{\textit{[why not go straight with defining ReD and ReC here, so it's also clear which U and L are used for each of these?]}}
where $U$ is ?free-stream? velocity, $L$ is characteristic length scales (?blade-chord length?), and $\nu$ is the fluid
kinematic viscosity.
...
\end{equation}\textbf{\textit{[define Uinf. it looks like a list of notation at the end of the paper could help a alot]}}
...
pictured in Figure~\ref{fig:tow-tank}\textbf{\textit{[missing fig?]}}. The turbine was mounted in a frame built
from NACA 0020 struts, attached to the tow carriage by four linear bearings,
which transfer all streamwise force to a pair of S-beam load cells \textbf{\textit{[need reference for each sensor - load cells, ADV, etc.]}}. The turbine
...
torque transducer. A load cell mounted, mounted at a fixed distance from the servo
motor, provides a redundant measurement. Turbine shaft angle was measured using
...
motion controller provided redundant measurements of the carriage speed \textbf{\textit{[need to say how well the agreement between these two measurements are and which one is used throughout the paper; else, remove one of these]}} turbine angular velocity sampled at 1 kHz. Turbine wake measurements at 1 meter
...
\begin{figure}[h]
    ...
    \caption{\textbf{Experimental setup.} Photo of the UNH tow tank and
        turbine test bed with RM2 installed.}
...
\begin{figure}[h]
    ...
    \textbf{\textit{[would be good to indicate the water surface with text of drawing ( e.g. a triangle and three lines]}}
    \label{fig:exp-setup}
\end{figure}
...
frame drag, respectively \textbf{\textit{[explain the purpose of these measurements; how the final drag and torque data were compensated using the tare measurements]}}. Tare drag runs were performed for each tow speed in
the experiment, for which the mean value was used in data processing\textbf{\textit{[describe how this was done - without blades but shaft?]}}. Tare
...
For each tow, a relevant quasi-steady duration was selected \textbf{\textit{[need a discussion on how this is done and justified]}}, and then truncated
...
reduction in the optimal tip speed ratio, due to stall delay \textbf{\textit{[need to explain what stall delay is]}}. These effect
diminish with increasing $Re$, which is expected as the blade boundary layers
transition from laminar to turbulent \textbf{\textit{[I wonder if this is relevant, since all cases are already in well in the turbulent regime]}} \cite{Lissaman1983,
Bachant2015-RVAT-Re-dep}. The rotor drag coefficient curves show similar
effects, but vary much less dramatically from each other.\textbf{\textit{[Overall I think we need more discussion for Figs 5 & 6.  For fig 5, maybe a discussion on stall delay and why Re independence is not achieve is sufficient.  Fig 6, need to find something to say, e.g. (1)why the trends look like that, is this expected? (2)Why they don't differ much between each other.    (3) mention that CD increases with ReD. ]}}
...
\textbf{\textit{[This figure shows that we haven't achieved Re independence (close though). I am sure many others will notice this.  Need to state why we couldn't go over Re = 1.3E6. I presume due to technical limitation.]}}
\label{fig:cp-curves}
\end{figure}
...
dramatically below $Re_D = 1 \times 10^6$ or $Re_c \approx 2 \times 10^5$ \textbf{\textit{[define Rec]}},
beyond which there appears to be a small, linear, positive trend \textbf{\textit{[dCP/dRe seems to increase again for the two points: second and third highest Cp - any suggestion for the cause of this?  On a second thought, I think it's more appropriate to plot Re Vs. max Cp here ( don't fix TSR to 3.1, but pick a value which gives max Cp]}} . The tendency
of $C_P$ to continue increasing slightly could be an effect of lower virtual
camber \textbf{\textit{[I think we need to elaborate more on this. On its own, this sentence sounds very speculative]}} when compared to the strong $Re$-independence of the higher $c/R$
...
lower solidity and tapered blades.\textbf{\textit{[the last 2 sentences: it's difficult to understand this without knowing the UNH-RVAT. Suggest to add a discussion on the similarity & differences between RVAT and RM2 dimensions, testing, and add the mean velocity plot for RVAT side-by-side with the one for RM2.  Need more details on how you see the effects of vortex shedding are weaker.]}}
...
$\lambda=3.1$.}\textbf{\textit{[is this looking downstream or upstream? may want to indicate the rotation direction of the downstream rotor side ( right or left)]}}
...
equation---analyzed identically to \cite{Bachant2015-JoT}\textbf{\textit{[Although this has been written in the cited reference, I feel we need to discuss this so that at least readers understand the reason of doing this analysis]}}---are shown in
...
given the inverse correlation between solidity and $\lambda_0$.\textbf{\textit{[need more to write here]}}
